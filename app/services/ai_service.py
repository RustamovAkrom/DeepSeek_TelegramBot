...
class AIService:
    ...
    async def generate(
        ...
        # Settings params
        max_tokens = min(
            max_tokens or self.max_tokens, 16000
        )  # безопасный лимит для free
        ...
        history_objs = await CRUDHistory.list_user_history(
            session, user_id=user.id, limit=settings.MAX_HISTORY
        )
        ...
        if save_history:
            # User request
            await CRUDHistory.add(
                session=session, user_id=user.id, role="user", content=text
            )
            # AI response
            await CRUDHistory.add(
                session=session, user_id=user.id, role="assistant", content=answer
            )
